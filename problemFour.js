...
const suppliesFound = ["insulin", "oatmeal", "raider", "zombie", "canned peaches"]

function raiderAvoidancePlan(targetsFoundWhileForaging) {
    for (let arrleng = 0; arrleng < targetsFoundWhileForaging.length; arrleng++) {
        if (targetsFoundWhileForaging[arrleng] === "raider" || targetsFoundWhileForaging[arrleng] === "zombie") {
            console.log("JARVIS has found a " + targetsFoundWhileForaging[arrleng] + "!! ENGAGING AVOIDANCE ACTION PLAN!!")
        } else { (console.log("JARVIS has found some " + targetsFoundWhileForaging[arrleng] + "!")) }
    }
}
raiderAvoidancePlan(suppliesFound)
console.log(suppliesFound.length)